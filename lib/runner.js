...
function getNestedValue(obj, path) {
  if (!path) return obj;
  return path.split(/\.(?![^\[]*\])|[\[\]]/).filter(Boolean).reduce((cur, prop) => cur?.[prop], obj);
}
...
function checkContainsWarning(status, assertion) {
  ...
  return warnings.some(w =>
    (typeof w === 'string' ? w : w.message || '').toLowerCase().includes(needle)
  );
}
...
function checkResolverInputsValid(resolverMeta) {
  ...
  return Array.isArray(inputs) && inputs.every(
    i => i && typeof i.name === 'string' && typeof i.type === 'string' && typeof i.required === 'boolean'
  );
}

function checkResolverOutputsValid(resolverMeta) {
  const outputs = resolverMeta.outputs;
  return Array.isArray(outputs) && outputs.every(
    o => o && typeof o.name === 'string' && typeof o.type === 'string'
  );
}
...
function checkResolverFailuresValid(resolverMeta) {
  ...
  return Array.isArray(failures) && failures.every(
    f => f && typeof f.code === 'string' && typeof f.retries === 'number'
  );
}
...
const assertionHandlers = {
  ...
  allowed_resolvers_listed: (ast, assertion) => checkAllowlist(ast, assertion.expected),
  resolver_names_normalized: (ast) => checkResolverNameNormalization(ast),
  workflow_name_present: (ast, assertion) => checkWorkflowName(ast, assertion.expected),
  workflow_return_values: (ast, assertion) => checkReturnValues(ast, assertion.expected),
  workflow_return_values_empty: (ast, assertion) => checkReturnValues(ast, assertion.expected),
  no_parse_warnings: (ast, assertion, status) => checkNoWarnings(status, assertion.expected),
  step_type: (ast, assertion) => checkStepType(ast, assertion),
  step_saveas: (ast, assertion) => checkStepSaveAs(ast, assertion),
  step_failure_policies: (ast, assertion) => checkStepFailurePolicies(ast, assertion),

  // Negative test assertions (R-004)
  contains_warning: (ast, assertion, status) => checkContainsWarning(status, assertion),
  status_greater_than: (ast, assertion, status) => checkStatusGreaterThan(status, assertion),

  // Resolver metadata assertions (R-005)
  resolver_has_field: (resolverMeta, assertion) => checkResolverHasField(resolverMeta, assertion),
  resolver_inputs_valid: (resolverMeta) => checkResolverInputsValid(resolverMeta),
  resolver_outputs_valid: (resolverMeta) => checkResolverOutputsValid(resolverMeta),
  field_names_normalized: (resolverMeta, assertion) => checkFieldNamesNormalized(resolverMeta, assertion),
  resolver_failures_valid: (resolverMeta) => checkResolverFailuresValid(resolverMeta),
};
...
function runAssertions(testSpec, target, status = {}) {
  ...
  for (const assertion of testSpec.assertions) {
    ...
    if (type in assertionHandlers) {
      ...
      failures.push({ id, severity, message: `Unknown assertion type: ${type}` });
      continue;
    }
...
async function runAllTests({ suites, resolver }) {
  ...
  for (const suite of suites) {
    ...
    // Determine test type: workflow parsing vs resolver metadata
    const fixture = testSpec.fixtures.inputs[0];
    if (fixture.workflow) {
      // --- Workflow parsing test (R-001 to R-004) ---
      ...
      try {
        const parseResult = resolver.parse ? resolver.parse(workflowSource) : { ast: resolver };
      ...
      try {
        // Load as JS module (assumes .ol is valid JS)
...
module.exports = { runAssertions, runAllTests };